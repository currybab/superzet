...
impl MentionUri {
    ...
    pub fn to_uri(&self) -> Url {
        match self {
            ...
            MentionUri::PastedImage => Url::parse("superzent:///agent/pasted-image").unwrap(),
            ...
            MentionUri::Selection {
                ...
                let mut url = if let Some(path) = abs_path {
                    ...
                    let mut url = Url::parse("superzent:///").unwrap();
            ...
            MentionUri::TerminalSelection { line_count } => {
                let mut url = Url::parse("superzent:///agent/terminal-selection").unwrap();
                url.query_pairs_mut()
                    .append_pair("lines", &line_count.to_string());
                url
            }
            MentionUri::GitDiff { base_ref } => {
                let mut url = Url::parse("superzent:///agent/git-diff").unwrap();
...
mod tests {
    ...
    fn test_parse_untitled_selection_uri() {
        let selection_uri = uri!("superzent:///agent/untitled-buffer#L1:10");
        let parsed = MentionUri::parse(selection_uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: None,
    ...
    fn test_parse_thread_uri() {
        let thread_uri = "superzent:///agent/thread/session123?name=Thread+name";
    ...
    fn test_parse_rule_uri() {
        let rule_uri =
            "superzent:///agent/rule/d8694ff2-90d5-4b6f-be33-33c1763acd52?name=Some+rule";
    ...
    fn test_parse_diagnostics_uri() {
        let uri = "superzent:///agent/diagnostics?include_warnings=true";
    ...
    fn test_parse_diagnostics_uri_warnings_only() {
        let uri = "superzent:///agent/diagnostics?include_warnings=true&include_errors=false";
        let parsed = MentionUri::parse(uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Diagnostics {
                include_errors,
    ...
    fn test_invalid_zed_path() {
        assert!(MentionUri::parse("superzent:///invalid/path", PathStyle::local()).is_err());
        assert!(MentionUri::parse("superzent:///agent/unknown/test", PathStyle::local()).is_err());
    }
    ...
    fn test_parse_terminal_selection_uri() {
        let terminal_uri = "superzent:///agent/terminal-selection?lines=42";
        ...
        // Test single line
        let single_line_uri = "superzent:///agent/terminal-selection?lines=1";
