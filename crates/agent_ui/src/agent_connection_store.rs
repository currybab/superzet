...
use gpui::{AppContext, Context, Entity, EventEmitter, SharedString, Subscription, Task, Window};
...
impl AgentConnectionStore {
    ...
    pub fn request_connection(
        ...
        self.entries.get(&key).cloned().unwrap_or_else(|| {
            let (mut new_version_rx, connect_task) =
                self.start_connection(server.clone(), window, cx);
    ...
    fn start_connection(
        ...
        let connect_task = cx.spawn_in(window, async move |_this, cx| match connect_task.await {
            Ok(connection) => match cx.update(|window, cx| {
                let history = connection.session_list(cx).map(|session_list| {
                    cx.new(|cx| ThreadHistory::new(Some(session_list), window, cx))
                });
